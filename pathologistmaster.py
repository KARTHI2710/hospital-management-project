...
import sqlite3
import os
from addpathologist import Ui_addpathologistForm
from editpathologist import Ui_editpathologistForm


class Ui_pathologistForm(object):
...
"")
        ...
        self.fetch_and_display_pathologist()

    def retranslateUi(self, Form):
        _translate = QtCore.QCoreApplication.translate
        Form.setWindowTitle(_translate("Form", "Form"))
        self.label.setText(_translate("Form", "Pathologist Master"))
        self.pushButton_5.setText(_translate("Form", "Add Pathologist"))
        ...
        self.pushButton_5.clicked.connect(self.addpathologist_form)
        self.pushButton_4.clicked.connect(self.filter_pathologist_data)



    def filter_pathologist_data(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(['DoctorCode','DoctorName','Qualification','Specification','Address','Mobile','Date','Signature','Actions'])
        #set header height
        ...
        pathologist_data = cursor.fetchall()

        count=len(pathologist_data)
        
        self.tableWidget.setRowCount(count)
        r=0
        c=0
        for row in pathologist_data:
            c=0
            for col in row:
            ...
            delete_button.clicked.connect(lambda _, docid=row[0]: self.delete_pathologist(docid))
            ...
            edit_button.clicked.connect(lambda _, docid=row[0]: self.edit_pathologist(docid))
    ...
    def fetch_and_display_pathologist(self):
        self.tableWidget.setColumnCount(9)
        self.tableWidget.setColumnWidth(0,80)
        self.tableWidget.setColumnWidth(1,100)
        self.tableWidget.setColumnWidth(2,150)
    ...
    def delete_pathologist(self, doc_code):
       ...
       cursor.execute("DELETE FROM pathologist WHERE DoctorCode = ?", (doc_code,))
       conn.commit()
    ...
    def addpathologist_form(self):
        self.add_pathologist_form = QtWidgets.QWidget()
        self.ui_addpathologist = Ui_addpathologistForm()
        self.ui_addpathologist.setupUi(self.add_pathologist_form)
        self.add_pathologist_form.show()
        self.ui_addpathologist.pushButton_4.clicked.connect(self.fetch_and_display_pathologist)



    def edit_pathologist(self, DoctorCode):
        self.edit_pathologist_form = QtWidgets.QWidget()
        self.ui_edit_pathologist = Ui_editpathologistForm()  # Replace with the correct class name
        self.ui_edit_pathologist.setupUi(self.edit_pathologist_form, DoctorCode)  # Pass the DoctorCode argument
        self.edit_pathologist_form.show()

        # Fetch pathologist data for the specified DoctorCode
        # pathologist_data = self.fetch_pathologist_data_by_id(DoctorCode)
        # if pathologist_data:
        #     self.edit_pathologist_form.pathologist_data = pathologist_data  
