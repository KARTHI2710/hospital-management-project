...
class Ui_addpathologistForm(object):
...
"")
        ...
        self.fetch_latest_pathologist_number()
        latest_pathologist_number = self.fetch_latest_pathologist_number()
        next_pathologist_number = self.generate_next_pathologist_number(latest_pathologist_number)
        self.lineEdit_25.setText(next_pathologist_number) 
    ...
    def fetch_latest_pathologist_number(self):
        try:
            self.cursor.execute("SELECT MAX(DoctorCode) FROM pathologist")
            latest_pathologist_number = self.cursor.fetchone()[0]
            return latest_pathologist_number
        except Exception as e:
            print("Error fetching latest pathologist number:", str(e))
            return None

    def generate_next_pathologist_number(self, latest_pathologist_number):
     if latest_pathologist_number is None:
         return "D00001"

     try:
         # Attempt to convert the numeric part to an integer
         numeric_part = int(latest_pathologist_number[1:])
     except ValueError:
         # Handle the case where the conversion fails (e.g., if latest_pathologist_number is not a valid integer string)
     ...
     next_pathologist_number = f"{prefix}{next_numeric_part:05}"  # Format as "D00001"
     return next_pathologist_number
